...
export class ExportService {
  ...
  async export(
    ...
    logger.info(`Exporting ${companies.length} companies as ${format}`);
   ...
   */
  private exportToCSV(companies: Company[], includeSignals: boolean): Buffer {
    // Define columns
    const columns = [
      { key: 'id', header: 'ID' },
    ...
    if (includeSignals) {
      columns.push(
        { key: 'signal_count', header: 'Signale_Anzahl' },
   ...
   */
  private exportToJSON(companies: Company[]): Buffer {
