...
class EnrichmentService {
    outscraper;
    constructor() {
        this.outscraper = new OutscraperClient_1.OutscraperClient();
    }
    /**
     * Enrich a batch of companies from raw Google Maps data
     */
    async enrichCompanies(rawResults, options = {}) {
        logger_1.logger.info(`Enriching ${rawResults.length} companies`);
        const companies = await Promise.all(rawResults.map((result) => this.enrichSingleCompany(result, options)));
     ...
     */
    async enrichSingleCompany(raw, options) {
     ...
     */
    calculateDealReadiness(raw, signals) {
        // Base score
        let score = 30;
        // Factors
        const factors = {
        ...
        if (raw.site)
            factors.financialHealth += 0.2;
        if (raw.photos_count > 10)
            factors.financialHealth += 0.1;
        if (raw.verified)
            factors.financialHealth += 0.1;
        // Owner Motivation (proxy: contact availability)
        if (raw.email)
            factors.ownerMotivation += 0.2;
        if (raw.phone)
            factors.ownerMotivation += 0.2;
        if (raw.site)
            factors.ownerMotivation += 0.1;
     ...
     */
    generateSummary(raw, signals, dealReadiness) {
        const parts = [];
        // Basic info
     ...
     */
    inferIndustry(category, subtypes) {
        const categoryMap = {
